...
export function ChatArea({ className, selectedChat, onBackToEmpty }: ChatAreaProps) {
  ...
  const knowledgeBasesUI = knowledgeBases.map(kb => ({
    ...
    icon: kb.id === 'rh' ? User : 
          kb.id === 'ops' ? Settings :
          kb.id === 'finance' ? Clock :
          kb.id === 'tech' ? BarChart3 : User,
    color: kb.id === 'rh' ? "bg-blue-100 text-blue-600" :
           kb.id === 'ops' ? "bg-purple-100 text-purple-600" :
           kb.id === 'finance' ? "bg-orange-100 text-orange-600" :
           kb.id === 'tech' ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-600"
  }))
